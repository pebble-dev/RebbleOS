...
uint8_t stm32_cc2264_init(const stm32_bluetooth_config_t *cc2264)
{
    ...
    stm32_power_request(_cc2264->shutdown_power, _cc2264->shutdown_power_port);
    ...
    */
    ...
    stm32_usart_init_device(_cc2264->usart);
    
    /* I'm going to request the usart stay on for now pending bluetooth */
    stm32_power_request(_cc2264->usart->config->usart_periph_bus, _cc2264->usart->config->usart_clock);
    ...
    stm32_power_release(_cc2264->shutdown_power, _cc2264->shutdown_power_port);
...
uint8_t stm32_cc2264_power_cycle(void)
{
    ...
    GPIO_ResetBits(_cc2264->shutdown_port, (1 << _cc2264->shutdown_pin_num));
    delay_ms(20);    
    GPIO_SetBits(_cc2264->shutdown_port, (1 << _cc2264->shutdown_pin_num));
    ...
    {
        if(GPIO_ReadInputDataBit(_cc2264->usart->config->gpio_ptr, _cc2264->usart->config->gpio_pin_cts_num) == 0)
        {
