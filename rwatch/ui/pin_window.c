...
PinWindow* pin_window_create(PinWindowCallbacks callbacks) {
  ...
  if (pin_window) {
    ...
    if (pin_window->window) {
      ...
      pin_window->main_text = text_layer_create(bounds);
      ...
      pin_window->sub_text = text_layer_create(bounds);
      ...
      pin_window->selection = selection_layer_create(bounds, PIN_WINDOW_NUM_CELLS);
      for (int i = 0; i < PIN_WINDOW_NUM_CELLS; i++) {
